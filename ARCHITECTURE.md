...
- Driving the per-file review loop. Removed files are skipped. Each file is reviewed inside its own error handler, so one failing file does not abort the others. Comments on lines that appear in the diff are posted as line comments; comments on other lines are listed in the review summary (the first 20, then a count of the rest), because GitHub rejects the entire review if any comment targets a line outside the diff. The summary also carries the first 20 of the scanner's findings, each with its file, line, description and recommendation; up to 10 de-duplicated style suggestions; any file whose model response could not be read; and any file that could not be reviewed. Text the model produced is flattened to one line, cut to 500 characters, and stripped of the markdown and HTML that would let it forge a heading or embed a link or image, because pull-request content is untrusted and the review is posted publicly.
...
- SQL injection (f-strings and `.format()` on a literal that looks like SQL, and `+` concatenation inside `execute(...)`, `executeQuery(...)` or onto a literal that starts with a SQL keyword). Every SQL rule first requires an uppercase clause keyword (`FROM`, `INTO`, `SET`, `VALUES`, `WHERE`) on the line, which is how SQL is written in code and how English prose such as `"Delete {} files from cache"`, and calls such as `execute("cd " + directory)`, are kept out. Lowercase SQL is missed; a false CRITICAL on someone's pull request costs more than a miss here.
...
Severity is assigned by class: SQL injection, command injection and path traversal are `CRITICAL`; hardcoded secrets and XSS are `HIGH`. No current class maps to `MEDIUM` or `LOW`, which exist for classes that may be added. The first 20 findings, ordered by severity then file and line, are listed in the review summary with their location and the pattern's recommendation, followed by a count of the rest.
...
`/health` returns liveness plus booleans saying whether Azure credentials and a GitHub token are configured, whether a webhook secret is set, and whether unsigned webhooks are allowed. A deployment with no Azure credentials, or with neither a secret nor `ALLOW_UNSIGNED_WEBHOOKS` (which rejects every delivery with 401), is therefore visible here rather than on the first pull request. The booleans say only that a value is set, never what it is.
