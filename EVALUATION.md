...
The harness lives in [`code_review_agent/evaluation/`](./code_review_agent/evaluation/). It runs end to end offline with `--mock-llm`, which the tests exercise; the live path uses the same code, and no live run is committed. The benchmark dataset is being built by hand and currently holds two illustrative fixtures. No results are reported. The methodology is written down before any results exist, and every change to it is logged below with its date and reason.
...
- **Ablation confounds.** See §3: with the bundled corpus and fixtures, the ablation cannot yet isolate an effect of selective retrieval.
...
| 2026-09-23 | An expected issue can narrow its own line tolerance, and `py-silent-except-002` uses ±1; reports name the file each scanner finding came from | A ±3 window around the bare `except:` covered the whole function, so any bug-category comment in it would have been credited |
